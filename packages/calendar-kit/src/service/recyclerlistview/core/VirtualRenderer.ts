...
export default class VirtualRenderer {
  ...
  private _recyclePool: RecycleItemPool | undefined;
  ...
  public syncAndGetKey(index: number): string {
    ...
    if (isNullOrUndefined(key)) {
      ...
      this._stableIdToRenderKeyMap[index] = { key };
    }
  ...
  private _onEngagedItemsChanged = (_all: number[], now: number[], notNow: number[]): void => {
