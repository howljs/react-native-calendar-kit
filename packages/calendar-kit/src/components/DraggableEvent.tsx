...
export const DraggableEvent: FC<DraggableEventProps> = ({
  ...
  }, [visibleDates, startUnix, columnWidthAnim]);
  ...
  }, [index, dragSelectedStartMinutes, start, minuteHeight, dragSelectedStartUnix, startUnix]);
  ...
  }, [eventHeight, eventWidth, isDragging, left, resourceIndex, top]);
...
const DraggableEventWrapper: FC<DraggableEventWrapperProps> = ({
  ...
  const { dragSelectedStartUnix, dragSelectedDuration, dragSelectedStartMinutes } = useDragEvent();
  ...
  const _handleDragSelectedEvent = (unix: number, minutes: number, duration: number) => {
