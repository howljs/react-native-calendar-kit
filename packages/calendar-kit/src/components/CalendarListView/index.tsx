...
const CalendarListView = forwardRef<CalendarListViewHandle, CalendarListViewProps>((props, ref) => {
  const {
    count,
    width,
    height,
  ...
  const layoutProvider = useMemo(() => new LayoutProvider({ width, height }), [width, height]);
  ...
  const baseOffsets = useMemo(() => {
    ...
    return Array.from({ length: columnsPerPage }, (_, col) => col * snapToInterval);
