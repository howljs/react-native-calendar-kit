...
import type { ResourceItem, UnavailableHourProps } from '../../types';
...
const UnavailableHours: FC<UnavailableHoursProps> = ({ visibleDates, resources }) => {
...
const UnavailableHoursByDate = memo(
  ({ currentUnix, visibleDateIndex, resources }: UnavailableHoursByDateProps) => {
...
UnavailableHoursByDate.displayName = 'UnavailableHoursByDate';
...
const UnavailableColumns = memo(
  ({ currentUnix, visibleDateIndex, unavailableHours, resources }: UnavailableColumnsProps) => {
    const { start: calendarStart, renderCustomUnavailableHour, numberOfDays } = useBody();
    ...
    const _renderSpecialRegion = (props: UnavailableColumnHourProps, regionIndex: number) => {
...
const UnavailableHourItem = ({
  ...
  const childWidth = useDerivedValue(
    ...
    [columnWidthAnim, widthPercentage]
  );
