...
export default class ViewabilityTracker {
  ...
  public forceRefresh(): boolean {
    const shouldForceScroll =
      this._actualOffset >= 0 && this._currentOffset >= this._maxOffset - this._windowBound;
  ...
  private _diffUpdateOriginalIndexesAndRaiseEvents(
    ...
    if (this.onVisibleColumnChanged && newVisibleItems.length !== 0 && this._columnsPerPage) {
      ...
      if (this._startColumn?.index !== startIndex || this._startColumn?.columns !== column) {
  ...
  private _calculateArrayDiff(arr1: number[], arr2: number[]): { now: number[]; notNow: number[] } {
