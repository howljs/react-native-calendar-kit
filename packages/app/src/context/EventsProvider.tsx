...
const EventsContext = React.createContext<Store<EventsState> | undefined>(undefined);
...
> = (
  ...
  const notifyDataChanged = useCallback(
    (date: number, offset: number = defaultOffset) => {
      ...
      const { packedAllDayEvents, packedAllDayEventsByDay, eventCountsByWeek, eventCountsByDay } =
  ...
  return <EventsContext.Provider value={eventStore}>{children}</EventsContext.Provider>;
};
...
export const useMonthEvents = (date: number, numberOfDays: number, visibleDays: number[]) => {
  const eventsContext = useContext(EventsContext);
